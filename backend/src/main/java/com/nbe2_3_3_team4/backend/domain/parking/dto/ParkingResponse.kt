...
class ParkingResponse {
    ...
    data class GetEnterParking(val orderId: String, val enterTime: String, val exitTime: String, val carNumber: String, val status: OrderStatus, val orderTime: String) {
        companion object {
            fun from(order: Order) = with(order) {
                ...
                GetEnterParking(
                    ...
                    orderDetail.startParkingTime!!.plusHours(ticket.parkingDuration!!.toLong()).format(timeFormatter),
